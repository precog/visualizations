...
class ufront_web_mvc_RedirectResult extends ufront_web_mvc_ActionResult {
	public function __construct($url, $permanentRedirect) {
		...
		if(null === $url) {
			throw new HException(new thx_error_NullArgument("url", "invalid null argument '{0}' for method {1}.{2}()", _hx_anonymous(array("fileName" => "RedirectResult.hx", "lineNumber" => 14, "className" => "ufront.web.mvc.RedirectResult", "methodName" => "new"))));
		}
		$this->url = $url;
	...
	public function executeResult($controllerContext) {
		if(null === $controllerContext) {
			throw new HException(new thx_error_NullArgument("controllerContext", "invalid null argument '{0}' for method {1}.{2}()", _hx_anonymous(array("fileName" => "RedirectResult.hx", "lineNumber" => 21, "className" => "ufront.web.mvc.RedirectResult", "methodName" => "executeResult"))));
		}
