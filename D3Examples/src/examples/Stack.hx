...
class Stack extends Example
{
	...
	override function runExample()
	{
		...
		bars.append("svg:rect")
			...
			.transition()
				...
				.attr("y").floatf(function(d, i) return y1(d))
				.attr("height").floatf(function(d, i) return y0(d) - y1(d));
	...
	{
		container.selectAll("g.layer rect")
			.transition()
				.duration(500)
				.delay(function(d, i) return (i % m) * 10)
				.attr("x").floatf(function(d, i) return x( { x : .9 * ~~(i / m) / n, y : 0.0, y0 : 0.0 } ))
				.attr("width").float(x( { x : .9 / n, y : 0.0, y0 : 0.0 } ))
				.endNode(transitionGroupEnd)
			;
	}
	...
	function transitionStack()
	{
		container.selectAll("g.layer rect")
			.transition()
				...
				.attr("y").floatf(function(d,i) return y1(d))
				.attr("height").floatf(function(d, i) return y0(d) - y1(d))
				.endNode(transitionStackEnd);
	}
	
	function transitionGroupEnd(d : Node<Dynamic>, i : Int)
	{
		Dom.selectNode(d)
			.transition()
				.duration(500)
				.attr("y").floatf(function(d,i) return h - y2(d))
				.attr("height").floatf(function(d,i) return y2(d));
	}
	
	function transitionStackEnd(d : Node<Dynamic>, i : Int)
	{
		Dom.selectNode(d)
			.transition()
				.duration(500)
