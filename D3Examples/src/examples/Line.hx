...
{
	override function runExample() // Array<Array<{x:Float,y:Float}>>
	{
		...
		var w = 450,
			h = 275,
			p = 20,
			x = new Linear().domain(0, 1).range(0, w),
			y = new Linear().domain(0, 1).range(h, 0);
		...
		vis.append("svg:path")
			...
			.attr("d").stringf(thx.svg.Line.pointObject()
	...
	override function description() return "Line chart. Not interactive."
}