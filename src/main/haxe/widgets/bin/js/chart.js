...
rg.pivottable.PivotTable.prototype.init = function() {
	if(null == this.availableProperties || 0 == this.availableProperties.length) rg.js.ReportGrid.children(this.path,{ property : this.event, type : "property"},$closure(this,"_loadProperties")); else this._init();
}
rg.pivottable.PivotTable.prototype.query = function() {
	...
	var properties = Arrays.filter(this.queryProperties,function(d) {
		...
		default:
			return (function($this) {
				var $r;
				throw new thx.error.Error("invalid property option {0}",null,d,{ fileName : "PivotTable.hx", lineNumber : 74, className : "rg.pivottable.PivotTable", methodName : "query"});
				return $r;
			}(this));
		}
	});
...
rg.pivottable.PivotTable.prototype.setAvailableProperties = function(values) {
	...
	this.availableProperties = values.map(function(d,i) {
...
rg.pivottable.PivotTable.prototype.startTimeChanged = function(n,_) {
	...
	if(v == "") this.start = null; else if(Dates.canParse(v)) this.start = Dates.parse(v); else {
		haxe.Log.trace("unable to parse date",{ fileName : "PivotTable.hx", lineNumber : 574, className : "rg.pivottable.PivotTable", methodName : "startTimeChanged"});
		return;
	}
	this.query();
}
rg.pivottable.PivotTable.prototype.endTimeChanged = function(n,_) {
	...
	if(v == "") this.end = null; else if(Dates.canParse(v)) this.end = Dates.parse(v); else {
		haxe.Log.trace("unable to parse date",{ fileName : "PivotTable.hx", lineNumber : 590, className : "rg.pivottable.PivotTable", methodName : "endTimeChanged"});
		return;
	}
	this.query();
}
...
rg.chart.LineChart = function(container,options) {
	if( container === $_ ) return;
	rg.chart.BaseChart.call(this,container,options);
}
rg.chart.LineChart.__name__ = ["rg","chart","LineChart"];
rg.chart.LineChart.__super__ = rg.chart.BaseChart;
for(var k in rg.chart.BaseChart.prototype ) rg.chart.LineChart.prototype[k] = rg.chart.BaseChart.prototype[k];
rg.chart.LineChart.prototype.data = null;
rg.chart.LineChart.prototype.xtime = null;
rg.chart.LineChart.prototype.chart = null;
rg.chart.LineChart.prototype.loading = null;
rg.chart.LineChart.prototype.initScales = function(options) {
...
rg.chart.LineChart.prototype.onAnimationStep = function() {
	this.x.domain([this.start,this.end]);
	this.refresh();
}
rg.chart.LineChart.prototype.onDataStep = function() {
...
rg.chart.LineChart.prototype.initChart = function(options) {
	this.data = [];
	var _g1 = 0, _g = this.query.values.length;
	while(_g1 < _g) {
		var i = _g1++;
...
rg.chart.LineChart.prototype.collectedData = null;
rg.chart.LineChart.prototype.seriesCount = null;
rg.chart.LineChart.prototype.updateData = function(pos,d) {
...
rg.chart.LineChart.prototype.tooltip = function(d,i) {
...
rg.chart.LineChart.prototype.updateTimeDomain = function() {
	var now = Date.now().getTime();
	this.start = now - 5400000;
	this.startdata = now - 5460000;
	this.end = now - 60000 * .5;
}
rg.chart.LineChart.prototype.currentMax = null;
rg.chart.LineChart.prototype.updateChart = function(data) {
...
rg.chart.LineChart.prototype.timeRange = function(data) {
	var sd = null;
...
rg.chart.LineChart.prototype.normalizeData = function(data) {
	...
	while(_g1 < _g) {
		...
		while(_g3 < _g2) {
			...
			if(null == pos) {
				haxe.Log.trace("invalid match for " + seq[j] + " should be between " + ticks[0] + " and " + ticks[ticks.length - 1] + " for " + this.periodicity,{ fileName : "LineChart.hx", lineNumber : 278, className : "rg.chart.LineChart", methodName : "normalizeData"});
				continue;
			}
...
rg.chart.LineChart.prototype.__class__ = rg.chart.LineChart;
...
rg.chart.StreamGraph = function(container,options) {
	if( container === $_ ) return;
	rg.chart.BaseChart.call(this,container,options);
}
rg.chart.StreamGraph.__name__ = ["rg","chart","StreamGraph"];
rg.chart.StreamGraph.__super__ = rg.chart.BaseChart;
for(var k in rg.chart.BaseChart.prototype ) rg.chart.StreamGraph.prototype[k] = rg.chart.BaseChart.prototype[k];
rg.chart.StreamGraph.prototype.data = null;
rg.chart.StreamGraph.prototype.xtime = null;
rg.chart.StreamGraph.prototype.chart = null;
rg.chart.StreamGraph.prototype.loading = null;
rg.chart.StreamGraph.prototype.initScales = function(options) {
	this.x = this.xtime = new thx.math.scale.LinearTime();
	this.x.range([0.0,this.width]);
	this.currentMax = 1.0;
	this.loading = false;
}
rg.chart.StreamGraph.prototype.onAnimationStep = function() {
	this.x.domain([this.start,this.end]);
	this.refresh();
}
rg.chart.StreamGraph.prototype.onDataStep = function() {
	if(this.loading) return;
	this.collectedData = [];
	this.seriesCount = 0;
	this.loading = true;
...
rg.chart.StreamGraph.prototype.initChart = function(options) {
	this.data = [];
	var _g1 = 0, _g = this.query.values.length;
	while(_g1 < _g) {
		var i = _g1++;
...
rg.chart.StreamGraph.prototype.collectedData = null;
rg.chart.StreamGraph.prototype.seriesCount = null;
rg.chart.StreamGraph.prototype.updateData = function(pos,d) {
	var a = Reflect.field(d,this.periodicity);
	this.collectedData[pos] = a;
	this.seriesCount++;
	if(this.seriesCount < this.query.values.length) return;
...
rg.chart.StreamGraph.prototype.tooltip = function(d,i) {
	var date = Date.fromTime(d.x);
	rg.chart.ToolTip.display("<span class=\"title\">" + this.query.values[i] + "</span><br/><span class=\"count\">" + "count: " + Ints.format(d.y) + "<br/>" + thx.culture.FormatDate.dateShort(date) + " " + thx.culture.FormatDate.timeShort(date) + "</span>");
}
rg.chart.StreamGraph.prototype.currentMax = null;
rg.chart.StreamGraph.prototype.updateChart = function(data) {
	if(null == this.chart) {
		this.chart = new rg.svg.SvgStreamGraph(this.space.center,this.x);
...
rg.chart.StreamGraph.prototype.timeRange = function(data) {
	var sd = null;
	var _g1 = 0, _g = data.length;
	while(_g1 < _g) {
		var i = _g1++;
...
rg.chart.StreamGraph.prototype.normalizeData = function(data) {
	var ticks = this.timeRange(data);
	if(null == ticks) ticks = this.xtime.timeTicks();
	var map = new Hash();
	var _g1 = 0, _g = ticks.length;
	...
	while(_g1 < _g) {
		...
		while(_g3 < _g2) {
			...
			var pos = map.get("" + seq[j][0]);
			if(null == pos) {
				haxe.Log.trace("invalid match for " + seq[j] + " should be between " + ticks[0] + " and " + ticks[ticks.length - 1] + " for " + this.periodicity,{ fileName : "StreamGraph.hx", lineNumber : 260, className : "rg.chart.StreamGraph", methodName : "normalizeData"});
				continue;
			}
			series[pos].y = seq[j][1];
		}
...
rg.chart.StreamGraph.prototype.__class__ = rg.chart.StreamGraph;
...
rg.Viz.makePivotOptions = function(pivot,options) {
	...
	pivot.setAvailableProperties(o.properties);
	...
	while(_g < _g1.length) {
		var property = _g1[_g];
		++_g;
		if(null != property.time) pivot.queryProperties.push(rg.pivottable.QueryProperty.TimeProperty(rg.Viz.periodicity(property.time),property.order.toLowerCase() == "top")); else pivot.queryProperties.push(rg.pivottable.QueryProperty.ValueProperty(property.name,property.order.toLowerCase() == "bottom",null == property.limit?20:property.limit));
	}
}
rg.Viz.periodicity = function(v) {
	if(null == v) return "Eternity";
	var v1 = Strings.capitalize(v.toLowerCase());
	if(!Reflect.hasField(rg.js.ReportGrid.Periodicity,v1)) throw new thx.error.Error("invalid periodicity '{0}'",null,v1,{ fileName : "Viz.hx", lineNumber : 74, className : "rg.Viz", methodName : "periodicity"});
	return v1;
}
rg.Viz.toDate = function(v) {
	...
	throw new thx.error.Error("invalid date value '{0}'",v,null,{ fileName : "Viz.hx", lineNumber : 86, className : "rg.Viz", methodName : "toDate"});
}
...
rg.Viz.line = function(el,options) {
	return new rg.chart.LineChart(rg.Viz.select(el),rg.Viz.makeoptions(options,{ }));
}
rg.Viz.stream = function(el,options) {
	return new rg.chart.StreamGraph(rg.Viz.select(el),rg.Viz.makeoptions(options,{ lineinterpolation : "cardinal", left : rg.Viz.scale(false,false,0)}));
}
rg.Viz.makeoptions = function(options,defaults) {
	var o = Objects.clone(rg.Viz.defaultOptions);
	Objects.copyTo(defaults,o);
	if(null == Reflect.field(options,"query")) throw new thx.error.Error("you have to provide a query for your data",null,null,{ fileName : "Viz.hx", lineNumber : 116, className : "rg.Viz", methodName : "makeoptions"});
	Objects.copyTo(options,o);
	return o;
}
rg.Viz.select = function(el) {
	var el1 = Std["is"](el,String)?thx.js.Dom.select(el):thx.js.Dom.selectNode(el);
	if(el1.empty()) throw new thx.error.Error("invalid container",null,null,{ fileName : "Viz.hx", lineNumber : 126, className : "rg.Viz", methodName : "select"});
	return el1;
}
...
rg.svg.SvgPieChart.prototype._redraw = function() {
	...
	var selection = vis.selectAll("g.group").dataf($closure(this._pie,"pie"),function(_,i) {
		return "" + i;
	});
...
rg.pivottable.PivotTable.startColor = thx.color.Hsl.ofRgb(thx.color.Rgb.fromInt(10092543));
rg.pivottable.PivotTable.endColor = thx.color.Hsl.ofRgb(thx.color.Rgb.fromInt(255));
