...
rg.chart.BaseChart = function(container,options) {
	if( container === $_ ) return;
	this.container = container;
	this.layers = [];
	this.initOptions(options,(function(f,a1) {
		return function() {
			return f(a1);
		};
	})($closure(this,"init"),options));
}
rg.chart.BaseChart.__name__ = ["rg","chart","BaseChart"];
rg.chart.BaseChart.prototype.container = null;
rg.chart.BaseChart.prototype.width = null;
rg.chart.BaseChart.prototype.height = null;
rg.chart.BaseChart.prototype.space = null;
rg.chart.BaseChart.prototype.x = null;
rg.chart.BaseChart.prototype.y = null;
rg.chart.BaseChart.prototype.timerAnimationUpdate = null;
rg.chart.BaseChart.prototype.timerDataUpdate = null;
rg.chart.BaseChart.prototype.periodicity = null;
rg.chart.BaseChart.prototype.lineInterpolator = null;
rg.chart.BaseChart.prototype.stacked = null;
rg.chart.BaseChart.prototype.query = null;
rg.chart.BaseChart.prototype.layers = null;
rg.chart.BaseChart.prototype.start = null;
rg.chart.BaseChart.prototype.startdata = null;
rg.chart.BaseChart.prototype.end = null;
rg.chart.BaseChart.prototype.onAnimationStep = function() {
}
rg.chart.BaseChart.prototype.onDataStep = function() {
}
rg.chart.BaseChart.prototype._lastanimation = null;
rg.chart.BaseChart.prototype._lastdata = null;
rg.chart.BaseChart.prototype._stepanimation = function(t) {
	if(t - this._lastanimation < this.timerAnimationUpdate) return false;
	this._lastanimation = t;
	this.updateTimeDomain();
	this.onAnimationStep();
	return false;
}
rg.chart.BaseChart.prototype._stepdata = function(t) {
	if(t - this._lastdata < this.timerDataUpdate) return false;
	this._lastdata = t;
	this.onDataStep();
	return false;
}
rg.chart.BaseChart.prototype.init = function(options) {
	this.updateTimeDomain();
	this.initScales(options);
	this.initSpace(options);
	this.initChart(options);
	this.refresh();
	if(this.timerAnimationUpdate > 0) {
		this._lastanimation = 0.0;
		thx.js.Timer.timer($closure(this,"_stepanimation"),this.timerAnimationUpdate);
	}
	if(this.timerDataUpdate > 0) {
		this._lastdata = 0.0;
		thx.js.Timer.timer($closure(this,"_stepdata"),this.timerDataUpdate);
	}
}
rg.chart.BaseChart.prototype.initScales = function(options) {
}
rg.chart.BaseChart.prototype.initOptions = function(options,handler) {
	this.width = options.width;
	this.height = options.height;
	this.query = options.query;
	this.timerAnimationUpdate = options.timeranimationupdate;
	this.timerDataUpdate = options.timerdataupdate;
	this.periodicity = "minute";
	this.stacked = options.stacked == true;
	var interp = null == options.lineinterpolation?null:options.lineinterpolation.split("-")[0];
	this.lineInterpolator = (function($this) {
		var $r;
		switch(interp) {
		case "basis":
		...
		case "cardinal":
			$r = (function($this) {
				...
				$r = null == v?thx.svg.LineInterpolator.Cardinal():thx.svg.LineInterpolator.Cardinal(Std.parseFloat(v));
				return $r;
			}($this));
			break;
		case "stepafter":
	...
	if(null == this.query.values) rg.js.ReportGrid.propertyValues(this.query.path,{ property : this.query.event + "." + this.query.property},(function(f,a1) {
		return function(a2) {
			return f(a1,a2);
		};
	})($closure(this,"loadProperties"),handler)); else handler();
}
rg.chart.BaseChart.prototype.loadProperties = function(handler,data) {
	this.query.values = data;
	handler();
}
rg.chart.BaseChart.prototype.initSpace = function(options) {
	this.space = new rg.svg.SvgSpace3x3(this.width,this.height,this.container,0);
	this.space.svg.attr("class").string("rg");
	this.space.setLeft(this.scalePanel(this.space.left,rg.svg.Anchor.Right,this.y,options.left));
	this.space.setRight(this.scalePanel(this.space.right,rg.svg.Anchor.Left,this.y,options.right));
	this.space.setTop(this.scalePanel(this.space.top,rg.svg.Anchor.Bottom,this.x,options.top));
	this.space.setBottom(this.scalePanel(this.space.bottom,rg.svg.Anchor.Top,this.x,options.bottom));
}
rg.chart.BaseChart.prototype.initChart = function(options) {
}
rg.chart.BaseChart.prototype.refresh = function() {
...
rg.chart.BaseChart.prototype.scalePanel = function(panel,anchor,scale,options) {
	var len = 0;
	if(options.ticks) {
		var tick = rg.svg.SvgScaleTick.ofLinear(panel,anchor,scale);
		this.layers.push(tick);
		tick.length(options.ticklength);
		len += options.ticklength;
	}
	if(options.labels) {
		if(options.ticks) len += options.spacing;
		var label = rg.svg.SvgScaleLabel.ofLinear(panel,anchor,scale);
		label.padding(len);
...
rg.chart.BaseChart.prototype.updateTimeDomain = function() {
	var now = Date.now().getTime();
	this.start = now - 900000;
	this.startdata = now - 960000;
	this.end = now - 60000 * .5;
}
rg.chart.BaseChart.prototype.__class__ = rg.chart.BaseChart;
rg.chart.StreamChart = function(container,options) {
	if( container === $_ ) return;
	rg.chart.BaseChart.call(this,container,options);
}
rg.chart.StreamChart.__name__ = ["rg","chart","StreamChart"];
rg.chart.StreamChart.__super__ = rg.chart.BaseChart;
for(var k in rg.chart.BaseChart.prototype ) rg.chart.StreamChart.prototype[k] = rg.chart.BaseChart.prototype[k];
rg.chart.StreamChart.prototype.data = null;
rg.chart.StreamChart.prototype.xtime = null;
rg.chart.StreamChart.prototype.chart = null;
rg.chart.StreamChart.prototype.loading = null;
rg.chart.StreamChart.prototype.initScales = function(options) {
	this.x = this.xtime = new thx.math.scale.LinearTime();
	this.x.range([0.0,this.width]);
	this.currentMax = 1.0;
	this.loading = false;
}
rg.chart.StreamChart.prototype.onAnimationStep = function() {
	this.x.domain([this.start,this.end]);
	this.refresh();
}
rg.chart.StreamChart.prototype.onDataStep = function() {
	if(this.loading) return;
	this.collectedData = [];
	this.seriesCount = 0;
	this.loading = true;
	var _g1 = 0, _g = this.query.values.length;
	while(_g1 < _g) {
		var i = _g1++;
		var o = { start : this.startdata, end : this.end, periodicity : "minute", property : this.query.event + "." + this.query.property, value : this.query.values[i]};
		rg.js.ReportGrid.propertyValueSeries(this.query.path,o,(function(f,a1) {
			return function(a2) {
				return f(a1,a2);
			};
		})($closure(this,"updateData"),i));
	}
}
rg.chart.StreamChart.prototype.initChart = function(options) {
	this.data = [];
	var _g1 = 0, _g = this.query.values.length;
	while(_g1 < _g) {
		var i = _g1++;
...
rg.chart.StreamChart.prototype.collectedData = null;
rg.chart.StreamChart.prototype.seriesCount = null;
rg.chart.StreamChart.prototype.updateData = function(pos,d) {
	var a = Reflect.field(d,this.periodicity);
	this.collectedData[pos] = a;
	this.seriesCount++;
	if(this.seriesCount < this.query.values.length) return;
	this.loading = false;
	var results = this.normalizeData(this.collectedData);
	this.updateChart(results);
}
rg.chart.StreamChart.prototype.tooltip = function(d,i) {
	var date = Date.fromTime(d.x);
	rg.chart.ToolTip.display("<span class=\"title\">" + this.query.values[i] + "</span><br/><span class=\"count\">" + "count: " + Ints.format(d.y) + "<br/>" + thx.culture.FormatDate.dateShort(date) + " " + thx.culture.FormatDate.timeShort(date) + "</span>");
}
rg.chart.StreamChart.prototype.currentMax = null;
rg.chart.StreamChart.prototype.updateChart = function(data) {
	if(null == this.chart) {
		this.chart = new rg.svg.SvgStreamChart(this.space.center,this.x);
		this.chart.setTooltip($closure(this,"tooltip"),function(d,i) {
			rg.chart.ToolTip.display();
		});
		if(null != this.lineInterpolator) this.chart.lineInterpolator(this.lineInterpolator);
		this.layers.push(this.chart);
	}
	this.chart.data(data);
	var len = Std["int"](Arrays.floatMax(data,function(a) {
		return a.length;
	}));
	var _g = 0;
	while(_g < len) {
		var i = _g++;
		var m = 0.0;
		var _g2 = 0, _g1 = data.length;
		while(_g2 < _g1) {
			var j = _g2++;
			m += data[j][i].y;
		}
		this.currentMax = Math.max(1,m);
	}
	this.chart.redraw();
}
rg.chart.StreamChart.prototype.timeRange = function(data) {
	var sd = null;
	...
	if(null == sd || null == sd[0] || null == sd[0][0]) return null;
	var s = sd[0][0], sample = s, d;
	switch(this.periodicity) {
	case "minute":
		while(sample > this.startdata) {
			s = sample;
			sample -= 60000;
		}
		break;
...
rg.chart.StreamChart.prototype.normalizeData = function(data) {
	var ticks = this.timeRange(data);
	if(null == ticks) ticks = this.xtime.timeTicks();
	var map = new Hash();
	...
	while(_g1 < _g) {
		...
		while(_g3 < _g2) {
			...
			if(null == pos) {
				haxe.Log.trace("invalid match for " + seq[j] + " should be between " + ticks[0] + " and " + ticks[ticks.length - 1] + " for " + this.periodicity,{ fileName : "StreamChart.hx", lineNumber : 260, className : "rg.chart.StreamChart", methodName : "normalizeData"});
				continue;
			}
...
rg.chart.StreamChart.prototype.__class__ = rg.chart.StreamChart;
Types = function() { }
...
thx.math.scale.NumericScale.prototype.invert = function(y,_) {
	return (y - this._range[0]) * this.ky + this._domain[0];
}
...
rg.pivottable.PivotTable.prototype.init = function() {
	if(null == this.availableProperties || 0 == this.availableProperties.length) rg.js.ReportGrid.children(this.path + "." + this.event,{ type : "property"},$closure(this,"_loadProperties")); else this._init();
}
rg.pivottable.PivotTable.prototype.query = function() {
	haxe.Log.trace(this.start,{ fileName : "PivotTable.hx", lineNumber : 48, className : "rg.pivottable.PivotTable", methodName : "query"});
}
rg.pivottable.PivotTable.prototype._init = function() {
	haxe.Log.trace(this.availableProperties,{ fileName : "PivotTable.hx", lineNumber : 53, className : "rg.pivottable.PivotTable", methodName : "_init"});
...
rg.pivottable.PivotTable.prototype.startTimeChanged = function(n,_) {
	var v = n.value;
	if(Dates.canParse(v)) this.start = Dates.parse(v); else {
		haxe.Log.trace("unable to parse date",{ fileName : "PivotTable.hx", lineNumber : 73, className : "rg.pivottable.PivotTable", methodName : "startTimeChanged"});
		return;
	}
	this.query();
}
rg.pivottable.PivotTable.prototype.endTimeChanged = function(n,_) {
	var v = n.value;
	if(Dates.canParse(v)) this.end = Dates.parse(v); else {
		haxe.Log.trace("unable to parse date",{ fileName : "PivotTable.hx", lineNumber : 87, className : "rg.pivottable.PivotTable", methodName : "endTimeChanged"});
		return;
	}
	this.query();
}
rg.pivottable.PivotTable.prototype._loadProperties = function(v) {
	this.availableProperties = v;
	this._init();
}
...
rg.chart.TimeChart = function(container,options) {
	if( container === $_ ) return;
	rg.chart.BaseChart.call(this,container,options);
}
rg.chart.TimeChart.__name__ = ["rg","chart","TimeChart"];
rg.chart.TimeChart.__super__ = rg.chart.BaseChart;
for(var k in rg.chart.BaseChart.prototype ) rg.chart.TimeChart.prototype[k] = rg.chart.BaseChart.prototype[k];
rg.chart.TimeChart.prototype.data = null;
rg.chart.TimeChart.prototype.xtime = null;
rg.chart.TimeChart.prototype.chart = null;
rg.chart.TimeChart.prototype.loading = null;
rg.chart.TimeChart.prototype.initScales = function(options) {
	this.x = this.xtime = new thx.math.scale.LinearTime();
	this.x.range([0.0,this.width]);
	this.y = new thx.math.scale.Linear().range([0.0,this.height]).domain([1.0,0.0]);
	this.currentMax = 1.0;
	this.loading = false;
}
rg.chart.TimeChart.prototype.onAnimationStep = function() {
	this.x.domain([this.start,this.end]);
	this.refresh();
}
rg.chart.TimeChart.prototype.onDataStep = function() {
	if(this.loading) return;
	this.collectedData = [];
	this.seriesCount = 0;
	this.loading = true;
...
rg.chart.TimeChart.prototype.initChart = function(options) {
	this.data = [];
	var _g1 = 0, _g = this.query.values.length;
	while(_g1 < _g) {
		var i = _g1++;
...
rg.chart.TimeChart.prototype.collectedData = null;
rg.chart.TimeChart.prototype.seriesCount = null;
rg.chart.TimeChart.prototype.updateData = function(pos,d) {
	var a = Reflect.field(d,this.periodicity);
	this.collectedData[pos] = a;
	this.seriesCount++;
	if(this.seriesCount < this.query.values.length) return;
...
rg.chart.TimeChart.prototype.tooltip = function(d,i) {
	var date = Date.fromTime(d.x);
	rg.chart.ToolTip.display("<span class=\"title\">" + this.query.values[i] + "</span><br/><span class=\"count\">" + "count: " + Ints.format(d.y) + "<br/>" + thx.culture.FormatDate.dateShort(date) + " " + thx.culture.FormatDate.timeShort(date) + "</span>");
}
rg.chart.TimeChart.prototype.currentMax = null;
rg.chart.TimeChart.prototype.updateChart = function(data) {
	var d = new thx.geom.layout.Stack().stack(data);
	if(null == this.chart) {
		var w = this.space.center.frame.width, h = this.space.center.frame.height;
		this.chart = new rg.svg.SvgLineChart(this.space.center,d,this.x,this.y);
		this.chart.setTooltip($closure(this,"tooltip"),function(d1,i) {
			rg.chart.ToolTip.display();
		});
		if(null != this.lineInterpolator) this.chart.lineInterpolator(this.lineInterpolator);
		this.chart.stacked(this.stacked);
		this.layers.push(this.chart);
	} else this.chart.data(d);
	if(this.chart.getStacked()) {
		var len = Std["int"](Arrays.floatMax(data,function(a) {
			return a.length;
		}));
		var _g = 0;
	...
	} else this.currentMax = Math.max(1,Arrays.floatMax(data,function(a) {
		return Arrays.floatMax(a,function(d1) {
			return d1.y;
		});
	}));
	this.y.domain([this.currentMax * 1.1,0]);
	this.chart.redraw();
}
rg.chart.TimeChart.prototype.timeRange = function(data) {
	var sd = null;
	var _g1 = 0, _g = data.length;
	while(_g1 < _g) {
		var i = _g1++;
...
rg.chart.TimeChart.prototype.normalizeData = function(data) {
	var ticks = this.timeRange(data);
	if(null == ticks) ticks = this.xtime.timeTicks();
	var map = new Hash();
	var _g1 = 0, _g = ticks.length;
	...
	while(_g1 < _g) {
		...
		while(_g3 < _g2) {
			...
			var pos = map.get("" + seq[j][0]);
			if(null == pos) {
				haxe.Log.trace("invalid match for " + seq[j] + " should be between " + ticks[0] + " and " + ticks[ticks.length - 1] + " for " + this.periodicity,{ fileName : "TimeChart.hx", lineNumber : 270, className : "rg.chart.TimeChart", methodName : "normalizeData"});
				continue;
			}
			series[pos].y = seq[j][1];
		}
...
rg.chart.TimeChart.prototype.__class__ = rg.chart.TimeChart;
...
thx.math.scale.LinearTime.prototype.periodicity = function(v) {
	v = v.toLowerCase();
	if(!Arrays.exists(thx.math.scale.LinearTime.valids,v)) throw new thx.error.Error("invalid periodicity '{0}'",null,v,{ fileName : "LinearTime.hx", lineNumber : 52, className : "thx.math.scale.LinearTime", methodName : "periodicity"});
	this._periodicity = v;
	return this;
}
...
rg.chart.PieChart = function(container,options) {
	if( container === $_ ) return;
	rg.chart.BaseChart.call(this,container,options);
}
rg.chart.PieChart.__name__ = ["rg","chart","PieChart"];
rg.chart.PieChart.__super__ = rg.chart.BaseChart;
for(var k in rg.chart.BaseChart.prototype ) rg.chart.PieChart.prototype[k] = rg.chart.BaseChart.prototype[k];
rg.chart.PieChart.prototype.data = null;
rg.chart.PieChart.prototype.loading = null;
rg.chart.PieChart.prototype.chart = null;
rg.chart.PieChart.prototype.initScales = function(options) {
	this.loading = false;
}
rg.chart.PieChart.prototype.initChart = function(options) {
	this.data = [];
	this.onDataStep();
}
rg.chart.PieChart.prototype.onAnimationStep = function() {
}
rg.chart.PieChart.prototype.onDataStep = function() {
	if(this.loading) return;
	this.loading = true;
	this.counter = 0;
	var _g1 = 0, _g = this.query.values.length;
	while(_g1 < _g) {
		var i = _g1++;
		var o = { periodicity : "eternity", property : this.query.event + "." + this.query.property, value : this.query.values[i]};
		rg.js.ReportGrid.propertyValueCount(this.query.path,o,(function(f,a1) {
			return function(a2) {
				return f(a1,a2);
			};
		})($closure(this,"updateData"),i));
...
rg.chart.PieChart.prototype.counter = null;
rg.chart.PieChart.prototype.updateData = function(pos,v) {
	this.data[pos] = v;
	if(++this.counter < this.query.values.length) return;
	this.loading = false;
	this.updateChart();
}
rg.chart.PieChart.prototype.updateChart = function() {
	if(null == this.chart) {
		this.chart = new rg.svg.SvgPieChart(this.space.center,this.data);
		var v = this.query.values;
		this.chart.setTooltip(function(d,i) {
			rg.chart.ToolTip.display("<span class=\"title\">" + v[i] + "</span><br/><span class=\"count\">" + Ints.format(d) + "</span>");
		},function(d,i) {
			rg.chart.ToolTip.display();
		});
	} else this.chart.data(this.data);
}
rg.chart.PieChart.prototype.__class__ = rg.chart.PieChart;
...
rg.svg.SvgLineChart.prototype._redrawn = null;
...
rg.svg.SvgLineChart.prototype._redraw = function() {
	...
	}).onNode("mousemove",$closure(this,"over")).onNode("mouseout",$closure(this,"out")).append("svg:path").attr("class").string("line").attr("d").stringf($closure(this,"path0")).transition().attr("d").stringf(this._path).endNode($closure(this,"_setPathCreated"));
	layer.exit().remove();
}
rg.svg.SvgLineChart.prototype._over = null;
rg.svg.SvgLineChart.prototype.over = function(n,i) {
	if(null == this._over) return;
	this._over(this.getDataAtNode(n),i);
}
rg.svg.SvgLineChart.prototype._out = null;
rg.svg.SvgLineChart.prototype.out = function(n,i) {
	if(null == this._out) return;
	this._out(this.getDataAtNode(n),i);
}
rg.svg.SvgLineChart.prototype.getDataAtNode = function(n) {
...
rg.svg.SvgLineChart.prototype.setTooltip = function(over,out) {
	this._over = over;
	this._out = out;
}
...
rg.Viz.pivot = function(el,options) {
	var pivot = new rg.pivottable.PivotTable(rg.Viz.select(el));
	rg.Viz.makePivotOptions(pivot,options);
	pivot.init();
	return pivot;
}
rg.Viz.makePivotOptions = function(pivot,options) {
	if(null == options) options = { };
	var o = Objects.copyTo(options,{ query : { path : null, event : null, properties : []}});
	pivot.availableProperties = o.properties;
	pivot.start = rg.Viz.toDate(o.query.start);
	pivot.end = rg.Viz.toDate(o.query.end);
	pivot.path = o.query.path;
	pivot.event = o.query.event;
	if(null == pivot.path) throw new thx.error.Error("you must provide a path value for your query",null,null,{ fileName : "Viz.hx", lineNumber : 46, className : "rg.Viz", methodName : "makePivotOptions"});
	if(null == pivot.event) throw new thx.error.Error("you must provide an event name for your query",null,null,{ fileName : "Viz.hx", lineNumber : 48, className : "rg.Viz", methodName : "makePivotOptions"});
	pivot.queryProperties = [];
	var _g = 0, _g1 = o.query.properties;
	while(_g < _g1.length) {
		var property = _g1[_g];
		++_g;
		if(null != property.time) pivot.queryProperties.push(rg.pivottable.QueryProperty.TimeProperty(rg.Viz.periodicity(property.time),property.order.toLowerCase() == "ascending",null == property.limit?20:property.limit)); else pivot.queryProperties.push(rg.pivottable.QueryProperty.ValueProperty(property.name,property.order.toLowerCase() == "ascending",null == property.limit?20:property.limit));
	}
}
rg.Viz.periodicity = function(v) {
	if(null == v) return "Eternity";
	var v1 = Strings.capitalize(v.toLowerCase());
	if(!Reflect.hasField(rg.js.ReportGrid.Periodicity,v1)) throw new thx.error.Error("invalid periodicity '{0}'",null,v1,{ fileName : "Viz.hx", lineNumber : 75, className : "rg.Viz", methodName : "periodicity"});
	return v1;
}
rg.Viz.toDate = function(v) {
	if(null == v || Std["is"](v,Date)) return null;
	if(Std["is"](v,Float)) return Date.fromTime(v);
	if(Std["is"](v,String) && Dates.canParse(v)) return Dates.parse(v);
	throw new thx.error.Error("invalid date value '{0}'",v,null,{ fileName : "Viz.hx", lineNumber : 87, className : "rg.Viz", methodName : "toDate"});
}
rg.Viz.pie = function(el,options) {
	return new rg.chart.PieChart(rg.Viz.select(el),rg.Viz.makeoptions(options,{ timeranimationupdate : 0, left : rg.Viz.scale(false,false,0), bottom : rg.Viz.scale(false,false,0)}));
}
rg.Viz.time = function(el,options) {
	return new rg.chart.TimeChart(rg.Viz.select(el),rg.Viz.makeoptions(options,{ }));
}
rg.Viz.stream = function(el,options) {
	return new rg.chart.StreamChart(rg.Viz.select(el),rg.Viz.makeoptions(options,{ lineinterpolation : "cardinal", left : rg.Viz.scale(false,false,0)}));
}
rg.Viz.makeoptions = function(options,defaults) {
	var o = Objects.clone(rg.Viz.defaultOptions);
	Objects.copyTo(defaults,o);
	if(null == Reflect.field(options,"query")) throw new thx.error.Error("you have to provide a query for your data",null,null,{ fileName : "Viz.hx", lineNumber : 117, className : "rg.Viz", methodName : "makeoptions"});
	Objects.copyTo(options,o);
	return o;
}
rg.Viz.select = function(el) {
	var el1 = Std["is"](el,String)?thx.js.Dom.select(el):thx.js.Dom.selectNode(el);
	if(el1.empty()) throw new thx.error.Error("invalid container",null,null,{ fileName : "Viz.hx", lineNumber : 127, className : "rg.Viz", methodName : "select"});
	return el1;
}
...
rg.pivottable.QueryProperty = { __ename__ : ["rg","pivottable","QueryProperty"], __constructs__ : ["ValueProperty","TimeProperty"] }
rg.pivottable.QueryProperty.ValueProperty = function(name,ascending,limit) { var $x = ["ValueProperty",0,name,ascending,limit]; $x.__enum__ = rg.pivottable.QueryProperty; $x.toString = $estr; return $x; }
rg.pivottable.QueryProperty.TimeProperty = function(periodicity,ascending,limit) { var $x = ["TimeProperty",1,periodicity,ascending,limit]; $x.__enum__ = rg.pivottable.QueryProperty; $x.toString = $estr; return $x; }
Floats = function() { }
...
rg.svg.SvgPieChart = function(panel,data) {
	if( panel === $_ ) return;
	this._padding = 4;
	rg.svg.SvgLayer.call(this,panel);
	this._data = data;
...
rg.svg.SvgPieChart.prototype._prepdata = null;
...
rg.svg.SvgPieChart.prototype.destroy = function() {
}
rg.svg.SvgPieChart.prototype.init = function() {
	this._r = Math.min(this.panel.frame.width,this.panel.frame.height) / 2;
	this._pie = new thx.geom.layout.Pie();
	this._arc = thx.svg.Arc.fromAngleObject().innerRadius((this._r - this._padding) * .5).outerRadius(this._r - this._padding);
}
...
rg.svg.SvgPieChart.prototype._redraw = function() {
	var vis = this.svg.classed().add("pie-chart").data([this._data]).update();
	var selection = vis.selectAll("g.layer").dataf($closure(this._pie,"pie"),function(_,i) {
		return "" + i;
	});
	selection.update().select("path").transition().attr("d").stringf($closure(this._arc,"shape"));
	selection.update().select("text").text().stringf(function(d,i) {
		return Ints.format(d.value);
	}).transition().attr("transform").stringf($closure(this,"tranformLabel")).attr("display").stringf(function(d,i) {
		return d.value > .15?null:"none";
	});
	var arcs = selection.enter().append("svg:g").attr("class").stringf(function(d,i) {
		return "layer layer-" + i;
	}).attr("transform").string("translate(" + (this._padding / 2 + this._r) + "," + (this._padding / 2 + this._r) + ")");
	arcs.on("mousemove",$closure(this,"over")).on("mouseout",$closure(this,"out"));
	arcs.append("svg:path").attr("d").stringf($closure(this._arc,"shape"));
	arcs.append("svg:text").attr("transform").stringf($closure(this,"tranformLabel")).attr("dy").string(".35em").attr("text-anchor").string("middle").attr("display").stringf(function(d,i) {
		return d.value > .15?null:"none";
	}).text().stringf(function(d,i) {
		return Ints.format(d.value);
	});
}
rg.svg.SvgPieChart.prototype._over = null;
rg.svg.SvgPieChart.prototype.over = function(d,i) {
	if(null != this._over) this._over(d.value,i);
}
rg.svg.SvgPieChart.prototype._out = null;
rg.svg.SvgPieChart.prototype.out = function(d,i) {
	if(null != this._out) this._out(d,i);
}
rg.svg.SvgPieChart.prototype.setTooltip = function(over,out) {
	this._over = over;
	this._out = out;
}
rg.svg.SvgPieChart.prototype.tranformLabel = function(d,i) {
...
Main.main = function() {
	var where = { };
	where[".tweet.startup"] = "@ReportGrid";
	rg.js.ReportGrid.searchSeries("/tweets",{ periodicity : "eternity", where : where},function(v) {
		haxe.Log.trace(v,{ fileName : "Main.hx", lineNumber : 17, className : "Main", methodName : "main"});
	});
	rg.js.ReportGrid.propertyCount("/tweets",{ property : ".tweet.startup"},function(v) {
		haxe.Log.trace("propertyCount tweets: " + v,{ fileName : "Main.hx", lineNumber : 18, className : "Main", methodName : "main"});
	});
	rg.js.ReportGrid.propertyValueCount("/tweets",{ property : ".tweet.startup", value : "@ReportGrid"},function(v) {
		haxe.Log.trace("propertyValueCount tweets: " + v,{ fileName : "Main.hx", lineNumber : 19, className : "Main", methodName : "main"});
	});
	rg.js.ReportGrid.propertyCount("/pivots",{ property : ".click.gender"},function(v) {
		haxe.Log.trace("propertyCount pivot: " + v,{ fileName : "Main.hx", lineNumber : 20, className : "Main", methodName : "main"});
	});
	rg.js.ReportGrid.propertyValueCount("/pivots",{ property : ".click.gender", value : "@male"},function(v) {
		haxe.Log.trace("propertyValueCount pivot: " + v,{ fileName : "Main.hx", lineNumber : 21, className : "Main", methodName : "main"});
	});
	rg.Viz.pivot("#pivot",{ query : { path : Main.PATH_PIVOTS, event : "click"}, properties : ["Gender","Location"]});
	Main.createTweetRandomData();
...
Main.createPivotData = function() {
	...
	while(_g < qt) {
		var i = _g++;
		rg.js.ReportGrid.track(Main.PATH_PIVOTS,{ count : 1, timestamp : now - Std.random(Main.TIME_SPAN), events : { click : { location : Arrays.random(Main.locations), gender : Arrays.random(Main.genders)}}});
	}
}
...
rg.svg.SvgStreamChart = function(panel,xscale) {
	if( panel === $_ ) return;
	this._cpid = "streamchart_clip_path_" + ++rg.svg.SvgStreamChart._pathid;
	rg.svg.SvgLayer.call(this,panel);
	this._scalex = xscale;
}
rg.svg.SvgStreamChart.__name__ = ["rg","svg","SvgStreamChart"];
rg.svg.SvgStreamChart.__super__ = rg.svg.SvgLayer;
for(var k in rg.svg.SvgLayer.prototype ) rg.svg.SvgStreamChart.prototype[k] = rg.svg.SvgLayer.prototype[k];
rg.svg.SvgStreamChart.prototype._data = null;
rg.svg.SvgStreamChart.prototype._prepdata = null;
rg.svg.SvgStreamChart.prototype._scalex = null;
rg.svg.SvgStreamChart.prototype._cpid = null;
rg.svg.SvgStreamChart.prototype._interpolator = null;
rg.svg.SvgStreamChart.prototype._area = null;
rg.svg.SvgStreamChart.prototype.destroy = function() {
}
rg.svg.SvgStreamChart.prototype.init = function() {
...
rg.svg.SvgStreamChart.prototype.lineInterpolator = function(interpolator) {
	this._interpolator = interpolator;
}
rg.svg.SvgStreamChart.prototype._redrawn = null;
rg.svg.SvgStreamChart.prototype.redraw = function() {
	this._redraw();
}
rg.svg.SvgStreamChart.prototype.data = function(d) {
	this._data = d;
	this._prepareData();
	this._redraw();
}
rg.svg.SvgStreamChart.prototype._h = null;
rg.svg.SvgStreamChart.prototype._w = null;
rg.svg.SvgStreamChart.prototype._prepareData = function() {
...
rg.svg.SvgStreamChart.prototype._transition = function() {
	var layer = this.svg.selectAll("g.layer").data(this._prepdata);
	layer.update().select("path.line").transition().attr("d").stringf($closure(this._area,"shape"));
}
rg.svg.SvgStreamChart.prototype._over = null;
rg.svg.SvgStreamChart.prototype.over = function(n,i) {
	if(null == this._over) return;
	this._over(this.getDataAtNode(n),i);
}
rg.svg.SvgStreamChart.prototype._out = null;
rg.svg.SvgStreamChart.prototype.out = function(n,i) {
	if(null == this._out) return;
	this._out(this.getDataAtNode(n),i);
}
rg.svg.SvgStreamChart.prototype.getDataAtNode = function(n) {
	var time = this._scalex.invert(thx.js.Svg.mouse(n)[0]);
	var data = Reflect.field(n,"__data__");
	var delta = Math.POSITIVE_INFINITY, pos = 0, v = Math.abs(time - data[0].x);
	while(v < delta) {
...
rg.svg.SvgStreamChart.prototype.setTooltip = function(over,out) {
	this._over = over;
	this._out = out;
}
rg.svg.SvgStreamChart.prototype._redraw = function() {
	this.svg.select("#" + this._cpid + " rect").attr("width")["float"](this._w).attr("height")["float"](this._h);
	var layer = this.svg.selectAll("g.layer").data(this._prepdata,function(d,i) {
		return "" + i;
	});
...
rg.svg.SvgStreamChart.prototype.__class__ = rg.svg.SvgStreamChart;
...
rg.Viz.hlen = 20;
rg.Viz.vlen = 50;
rg.Viz.defaultOptions = { width : 400, height : 400, query : null, left : rg.Viz.scale(true,true,rg.Viz.hlen), right : rg.Viz.scale(false,false,rg.Viz.hlen), top : rg.Viz.scale(false,false,rg.Viz.vlen), bottom : rg.Viz.scale(true,true,rg.Viz.vlen), timeranimationupdate : 250, timerdataupdate : 10000, lineinterpolation : null, stacked : false};
...
Main.locations = ["El Paso","Denver","Arapahoe","Jefferson","Adams","Larimer","Boulder","Douglas","Weld","Pueblo","Mesa","Garfield"];
Main.genders = ["male","female"];
Main.gluecons = ["@ReportGrid","@ReportGrid","@ReportGrid","@ReportGrid","@One","@One","@Two","@Two","@Three","@Four","@Five","@Six","@Seven","@Eight","@Nine","@Nine","@Ten","@Eleven","@Eleven","@Twelve","@Thirteen","@Fourteen"];
rg.svg.SvgStreamChart._pathid = 0;
