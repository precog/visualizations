...
class TestFactoryAxis 
{
	public function testAxisTime() 
	{
		...
		var axis = Types.as(factory.createDiscrete(Properties.timeProperty("hour"), [1, 2, 3]), AxisTime);
		Assert.notNull(axis);
	...
	{
		var factory = new FactoryAxis();
		var axis = Types.as(factory.createDiscrete("count", [1, 2, 3]), AxisOrdinal);
		Assert.equals(1, axis.first);
		Assert.equals(3, axis.last);
		Assert.same(Set.ofArray([1, 2, 3]), axis.values);
		
		axis = Types.as(factory.createDiscrete("count", null), AxisOrdinal);
