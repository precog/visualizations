...
class FactoryDataSource 
{
	...
	function createFromQuery(path : String, event : String, query : Null<String>, groupby : Null<String>, start : Null<Float>, end : Null<Float>)
	{
		if (null == query)
			query = "";
		return new DataSourceReportGrid(executor, path, event, parser.parse(query), groupby, start, end);
	}
}