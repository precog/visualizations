...
rg.controller.visualization.VisualizationBarChart.prototype = $extend(rg.controller.visualization.VisualizationCartesian.prototype,{
	...
	,transformData: function(dps) {
		haxe.Log.trace(dps,{ fileName : "VisualizationBarChart.hx", lineNumber : 1, className : "rg.controller.visualization.VisualizationBarChart", methodName : "transformData"});
		var results = [], variable = this.independentVariables[0], values = variable.axis.range(variable.min(),variable.max());
		haxe.Log.trace(variable,{ fileName : "VisualizationBarChart.hx", lineNumber : 1, className : "rg.controller.visualization.VisualizationBarChart", methodName : "transformData"});
		haxe.Log.trace(values,{ fileName : "VisualizationBarChart.hx", lineNumber : 1, className : "rg.controller.visualization.VisualizationBarChart", methodName : "transformData"});
		var _g = 0;
		...
		haxe.Log.trace(results,{ fileName : "VisualizationBarChart.hx", lineNumber : 1, className : "rg.controller.visualization.VisualizationBarChart", methodName : "transformData"});
		return results;
	}
...
rg.view.svg.chart.Sankey.prototype = $extend(rg.view.svg.chart.Chart.prototype,{
	...
	,redraw: function() {
		...
		backedgesy += this.minCurve + this.padFlow;
		this.padBefore = 0.0;
		var _g = 0, _g1 = this.layout.layer(0);
		while(_g < _g1.length) {
			var node = _g1[_g];
			++_g;
			var extra = Math.min(this.nheight(node.data.extrain),this.maxFalloffWidth);
		...
		while(_g < _g1.length) {
			var node = _g1[_g];
			++_g;
			var extra = Math.min(this.nheight(node.data.extraout),this.maxFalloffWidth);
			var $it2 = node.graph.edges.positives(node);
			while( $it2.hasNext() ) {
				var edge = $it2.next();
				var head = edge.head, childweight = this.hafter(edge.id,node.graph.edges.positives(node)) + this.nheight(edge.weight) + Math.min(this.nheight(node.data.extraout),this.maxFalloffWidth);
...
rg.controller.info.InfoDataSource.filterDate = function(v) {
	...
	return (function($this) {
		var $r;
		throw new thx.error.Error("invalid date '{0}' for start or end",[v],null,{ fileName : "InfoDataSource.hx", lineNumber : 1, className : "rg.controller.info.InfoDataSource", methodName : "filterDate"});
		return $r;
	}(this));
}
...
rg.view.svg.chart.BarChart.prototype = $extend(rg.view.svg.chart.CartesianChart.prototype,{
	...
	,data: function(dps) {
		haxe.Log.trace(dps,{ fileName : "BarChart.hx", lineNumber : 1, className : "rg.view.svg.chart.BarChart", methodName : "data"});
...
rg.controller.factory.FactoryDataSource.prototype = {
	...
	,create: function(info) {
		...
		throw new thx.error.Error("to create a query you need to reference by name an existing data source or provide  at least the data and the name or the event and the path parameters",null,null,{ fileName : "FactoryDataSource.hx", lineNumber : 1, className : "rg.controller.factory.FactoryDataSource", methodName : "create"});
	}
...
rg.controller.visualization.VisualizationSankey.prototype = $extend(rg.controller.visualization.VisualizationSvg.prototype,{
	...
	,layoutData: function(data,idf,nodef,weightf,edgesf) {
		...
		if(rg.controller.visualization.VisualizationSankey.REMOVEME) {
			rg.controller.visualization.VisualizationSankey.REMOVEME = false;
...
rg.controller.visualization.VisualizationSankey.REMOVEME = true;
