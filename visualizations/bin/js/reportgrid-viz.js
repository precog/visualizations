...
rg.view.svg.chart.Sankey.prototype = $extend(rg.view.svg.chart.Chart.prototype,{
	...
	,redraw: function() {
		...
		while(_g < _g1.length) {
			...
			haxe.Log.trace(extrain,{ fileName : "Sankey.hx", lineNumber : 107, className : "rg.view.svg.chart.Sankey", methodName : "redraw"});
			if(node.children.length > 0) {
				...
				haxe.Log.trace(childWeight,{ fileName : "Sankey.hx", lineNumber : 111, className : "rg.view.svg.chart.Sankey", methodName : "redraw"});
		...
		var normMin = function(v) {
			return Math.max(0,Math.min(v - 3,5));
		};
		...
		while(_g < _g1.length) {
			...
			while(_g2 < level.length) {
				...
				elbow.update(rg.view.svg.widget.Orientation.RightBottom,falloff,this.levelWidth / 2 + this.xlevel(node.level),this.ynode(node) + this.ydiagonal(null,node.children) + falloff,normMin(falloff),this.maxFalloffWidth,0,5);
			}
		}
		var _g = 0, _g1 = this.layout;
		while(_g < _g1.length) {
			...
			while(_g2 < level.length) {
				...
				haxe.Log.trace(extra,{ fileName : "Sankey.hx", lineNumber : 207, className : "rg.view.svg.chart.Sankey", methodName : "redraw"});
				elbow.update(rg.view.svg.widget.Orientation.LeftTop,extra,-this.levelWidth / 2 + this.xlevel(node.level),this.ynode(node),normMin(extra),this.maxFalloffWidth,0,5);
			}
		}
...
rg.view.svg.widget.Map.prototype = {
	...
	,loadGeoJsonAjax: function(path) {
		...
		http.onError = function(err) {
			throw new thx.error.Error("unable to load GeoJSON file '{0}': {1}",[path,err],null,{ fileName : "Map.hx", lineNumber : 1, className : "rg.view.svg.widget.Map", methodName : "loadGeoJsonAjax"});
		};
		http.request(false);
	}
	,draw: function(json) {
...
rg.view.svg.widget.HookConnectorArea.prototype = {
	...
	,update: function(x1,y1,x2,y2,weight,yreference,before,after) {
		var upperp = this.createPath(x1,y1,x2,y2,y1 > yreference?yreference:yreference + weight,before + weight,after + weight), lowerp = this.createPath(x2,y2 + weight,x1,y1 + weight,y1 > yreference?yreference - weight:yreference,-after,-before);
		this.upper.attr("d").string(upperp);
		this.lower.attr("d").string(lowerp);
		this.area.attr("d").string(upperp + "L" + lowerp.substr(1) + "z");
	}
	,createPath: function(x1,y1,x2,y2,yref,before,after) {
		var path = "M" + x1 + "," + y1;
		path += "L" + (x1 + before) + "," + y1;
...
rg.data.source.DataSourceReportGrid = $hxClasses["rg.data.source.DataSourceReportGrid"] = function(executor,path,event,query,groupby,timezone,start,end) {
	...
	case 0:
		break;
	default:
		throw new thx.error.Error("RGDataSource doesn't support operation '{0}'",null,this.operation,{ fileName : "DataSourceReportGrid.hx", lineNumber : 1, className : "rg.data.source.DataSourceReportGrid", methodName : "new"});
	}
	this.path = path;
	...
	,unit: function() {
		return (function($this) {
			...
			default:
				$r = (function($this) {
					var $r;
					throw new thx.error.Error("unsupported operation '{0}'",null,$this.operation,{ fileName : "DataSourceReportGrid.hx", lineNumber : 1, className : "rg.data.source.DataSourceReportGrid", methodName : "unit"});
					return $r;
				}($this));
			}
			return $r;
...
rg.view.svg.widget.DiagonalArea.prototype = {
	...
	,update: function(x1,y1,x2,y2,sw,ew) {
		...
		haxe.Log.trace(path,{ fileName : "DiagonalArea.hx", lineNumber : 1, className : "rg.view.svg.widget.DiagonalArea", methodName : "update"});
...
rg.data.source.rgquery.QueryParser.parseValue = function(s) {
	...
	return (function($this) {
		var $r;
		throw new thx.error.Error("invalid value '{0}'",null,s,{ fileName : "QueryParser.hx", lineNumber : 1, className : "rg.data.source.rgquery.QueryParser", methodName : "parseValue"});
		return $r;
	}(this));
}
